            ...
            slider ?
                <div key={index} className={"kasko-offer__slide slider " + (credit ? " credit" : "")}>
                    <div ref={this.setWrapperRef}
                         className={"kasko-offer__item" + (offer.collapse ? " collapsable" : "") + ((this.state.offerAdded) && !completed ? " active" : "") + (completed ? " completed" : "") + ((this.state.offerCollapsed) ? " collapsed" : "")}>
                        <div
                            onClick={(e) => {
                        ...
                        {(offer.collapse && !this.state.offerCollapsed) ?
                            offer.dropdown === 'KaskotaxPopup' ?
                                <div className="kasko-offer__item--info wide">
                                    <KaskotaxPopup toggleFunc={(e, action) => {
                                        this.toggleOfferAdded(e, index, offer)
                                    }} updatePaymentState={this.dropdownCallback} popupCloseFunc={this.dropdownClose}
                                ...
                                : <div className="kasko-offer__item--info">
                                    ...
                                    <InputNumber
                                        ...
                                        onChange={(value) => {
                                            this.onPeriodChange(value, index)
                                        }}
                                    />
                ...
                <Col span={6} key={index}>
                    {offer.collapse ?
                        <div key={index} className={"kasko-offer__slide" + (credit ? " credit" : "")}>
                            <div ref={this.setWrapperRef}
                                 className={"kasko-offer__item" + (offer.collapse ? " collapsable" : "") + ((active || this.state.offerAdded) && !completed ? " active" : "") + (completed ? " completed" : "") + ((this.state.offerCollapsed) ? " collapsed" : "")}>
                                <div
                                    onClick={(e) => ((offer.func && typeof offer.func === 'function') ? offer.func() : offer.href ? this.goTo(offer, offer.goto) : (this.toggleActiveOffer(e, index)))}
                                    ...
                                    {offer.button ?
                                        ...
                                        <Tooltip overlayClassName="tooltip_v1" placement="top"
                                                 title={this.state.offerAdded ? "Удалить" : "Добавить"}>
                                            <span onClick={(e) => this.toggleOfferAdded(e, index, offer)}
                                                  className={"kasko-offer__item--toggle"}/>
                                        </Tooltip>
                                    }
                                    <span className="kasko-offer__item--name">{offer.name}</span>
                                </div>
                                <div className="kasko-offer__item--body">
                                    {this.state.newPrice ? '' : offer.prefix}&nbsp;
                                    <span
                                        className="kasko-offer__item--price">{(prefix ? prefix + ' ' : '') + price}</span>
                                    &nbsp;{offer.suffix}
                                </div>
                                ...
                                {(offer.collapse && !this.state.offerCollapsed) ?
                                    <div className="kasko-offer__item--info">
                                        ...
                                        <div className="text_center">
                                            <div onClick={() => this.onShowOfferChange(index)}
                                                 className="kasko-offer__item--info-close">Свернуть
                                            </div>
                                        </div>
                                    </div>
                                    : null
                        ...
                        <Link ref={(el) => {
                            this.node = el
                        }} to={offer.link ? offer.link : "/offers"}
