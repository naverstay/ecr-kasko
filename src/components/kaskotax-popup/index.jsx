...
class KaskotaxPopup extends Component {
    ...
    render() {
        ...
        let calcMode = <Row gutter={20}
                            className="kasko-car-select__controls mb_20 ant-row-center ant-space-align-center">
            <Col span={10}>
                <Button onClick={() => {
                    this.popupAction()
            ...
            <Col span={10} className="text_center">
                {added ?
                    <span onClick={(e) => {
                        this.popupCancel(e, {creditAdd: false})
                    }} className="gl_link clr_gray">Убрать из кредита</span>
                    : <Button onClick={(e) => {
                        this.popupCancel(e, {creditAdd: true})
                    }} className={"w_100p fz_14 ant-btn-primary"} disabled={this.state.formBusy ? "disabled" : null}
